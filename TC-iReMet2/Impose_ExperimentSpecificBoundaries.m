function [ ExpCondBoundaries ] = Impose_ExperimentSpecificBoundaries( ModelParameter, KOeffects, ExpDataSpecific, day, model, AnalysisParameters )
    ...
    % MUTANT
    %%% CP changed everything so the outcome always is 'Bio_opt'
    if ~isempty(strfind(ExpDataSpecific.description{1, day}, AnalysisParameters.experimentname)) 
		...
		biomRxnM   = 'Bio_opt'; % CP changed just to bioopt to be save
		biomRxnIdM = find(strcmp(model.rxns, 'Bio_opt')); % CP changed to bioopt to be save 
    end
    % WILDTYPE
    if ~isempty(strfind(ExpDataSpecific.description{2, day}, AnalysisParameters.experimentname))
		...
		biomRxnWT   = 'Bio_opt'; % CP changed just to bioopt to be save
		biomRxnIdWT = find(strcmp(model.rxns, 'Bio_opt')); % CP changed to bioopt to be save 
    end             
...
%}
    ...
    if isequal(AnalysisParameters.with_rangeBiom,1)
        ...
        biomassFraction_time = table2array(readtable('/home/mpimp-golm.mpg.de/pries1347/winhome/main/MAIN-uebergabe/TC-iReMet2/Data/percentageOfBiomassProductionAtEachTimepoint2.csv'));
