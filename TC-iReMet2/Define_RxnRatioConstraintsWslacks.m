...
% CP here add transcript ratio once
        if isequaln(measured_T_ratio_max(rxn),NaN)
            total_ratio_max = total_ratio_max*(1*allExpT_ratio_max);  % CP - if Tratio is unmeasured 
        else
            total_ratio_max = total_ratio_max*(1*measured_T_ratio_max(rxn));  % CP - if Tratio is measured
        end
        %total_ratio_max = total_ratio_max*(1*measured_T_ratio_max(rxn));  % CP - HOW TO SET DEVIATION MULTIPLIER
        ...
        if total_ratio_max < AP.max_ratio  % To prevent numerical instabilites, constraints with too large values are neglected
            ...
            if AP.with_slacks == 1
                ...
                % slack entry, CP upper part original, changed to -1 so the
            ...
            if isequaln(measured_T_ratio_min(rxn),NaN)
                total_ratio_min = total_ratio_min*(1*allExpT_ratio_min);  % CP - if Tratio is unmeasured 
            else
                total_ratio_min = total_ratio_min*(1*measured_T_ratio_min(rxn));  % CP - if Tratio is measured
            end
            %total_ratio_min = total_ratio_min*(1*measured_T_ratio_min(rxn)); % CP 
    ...
    %writetable(totalmaxratiotable,'CP_totalmaxratiotable'); % CP 
end